...
const SyllabusVideoGenerator = () => {
  ...
  const subjects = [
    ...
    'Music', 'Physical Education', 'Economics', 'Environmental Science'
  ];
            ...
            if (response.success) {
              ...
              if (status === 'succeed' || status === 'completed') {
                ...
                setSyllabi(prev => prev.map(s => 
                  s._id === syllabusId 
                    ? { ...s, videoGenerationStatus: 'completed', videoUrl, thumbnailUrl }
                    : s
                ));
                // Remove from polling
                ...
                setGeneratingVideo(prev => ({ ...prev, [syllabusId]: false }));
  ...
  const handleCreateSyllabus = async () => {
    try {
      if (!formData.title || !formData.subject || !formData.grade || !formData.content) {
      ...
      if (response.success) {
        ...
        resetForm();
        setSuccessMessage('Syllabus created successfully!');
      }
    } catch (err) {
  ...
  const handleUpdateSyllabus = async () => {
    try {
      ...
      if (response.success) {
        setSyllabi(syllabi.map(s => s._id === selectedSyllabus._id ? response.data : s));
  ...
  const handleGenerateVideo = async (syllabus) => {
    try {
      ...
      if (response.success) {
        ...
        setSyllabi(prev => prev.map(s => 
          s._id === syllabus._id 
            ? { ...s, videoGenerationStatus: 'generating', videoTaskId: response.data.taskId }
            : s
        ));
        ...
        setSuccessMessage('Video generation started! This may take 1-2 minutes.');
      }
    } catch (err) {
  ...
  const openEditModal = (syllabus) => {
    ...
    setFormData({
      ...
      topics: syllabus.topics?.length > 0 ? syllabus.topics : [{ topicName: '', description: '', duration: '' }]
    });
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div>
            ...
            <p style={{ color: '#6b7280' }}>Create syllabi and generate animated educational videos using AI</p>
          </div>
          <button
            onClick={() => {
              resetForm();
        ...
        {successMessage && (
          <div style={{
            ...
            <button onClick={() => setSuccessMessage('')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.25rem' }}>×</button>
          </div>
        )}
        {error && (
          <div style={{
        ...
        <div style={{
          ...
          {[
            { label: 'Total Syllabi', value: syllabi.length, color: '#3b82f6', icon: BookOpen },
            { label: 'Videos Generated', value: syllabi.filter(s => s.videoGenerationStatus === 'completed').length, color: '#22c55e', icon: Video },
            { label: 'In Progress', value: syllabi.filter(s => s.videoGenerationStatus === 'generating').length, color: '#f59e0b', icon: Loader },
            { label: 'Pending', value: syllabi.filter(s => s.videoGenerationStatus === 'pending' || !s.videoGenerationStatus).length, color: '#8b5cf6', icon: Clock }
          ].map((stat, index) => {
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '3rem' }}>
            <Loader size={48} className="animate-spin" style={{ color: '#3b82f6', margin: '0 auto' }} />
            <p style={{ color: '#6b7280', marginTop: '1rem' }}>Loading syllabi...</p>
          </div>
        ) : syllabi.length === 0 ? (
          <div style={{
            backgroundColor: 'white',
            ...
            <p style={{ color: '#6b7280', marginBottom: '1.5rem' }}>
              Create your first syllabus to get started generating educational videos
            </p>
            <button
              onClick={() => {
                resetForm();
                ...
                <div style={{
                  ...
                  {syllabus.videoUrl ? (
                    ...
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <p style={{ color: '#6b7280', marginTop: '0.5rem' }}>Generating video...</p>
                    </div>
                  ) : (
                    <div style={{ textAlign: 'center' }}>
                      <Video size={48} color="#9ca3af" />
                      <p style={{ color: '#6b7280', marginTop: '0.5rem' }}>No video yet</p>
                    </div>
                  )}
                  {/* Status Badge */}
                ...
                <div style={{ padding: '1.25rem' }}>
                  ...
                  <div style={{
                    ...
                    <button
                      ...
                      disabled={generatingVideo[syllabus._id] || syllabus.videoGenerationStatus === 'generating'}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '0.25rem',
                        padding: '0.5rem 0.75rem',
                        backgroundColor: generatingVideo[syllabus._id] || syllabus.videoGenerationStatus === 'generating' ? '#9ca3af' : '#3b82f6',
                        color: 'white',
                        border: 'none',
                        borderRadius: '0.375rem',
                        cursor: generatingVideo[syllabus._id] || syllabus.videoGenerationStatus === 'generating' ? 'not-allowed' : 'pointer',
                        fontSize: '0.875rem'
                      }}
                    >
                      {generatingVideo[syllabus._id] || syllabus.videoGenerationStatus === 'generating' ? (
                        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <label style={{ fontWeight: '500', color: '#374151' }}>Topics</label>
                    <button
                      onClick={addTopic}
                      style={{
                        display: 'flex',
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
                ...
                <button
                  onClick={showCreateModal ? handleCreateSyllabus : handleUpdateSyllabus}
                  style={{
                    flex: 1,
                    padding: '0.75rem',
                    backgroundColor: '#3b82f6',
        ...
        {showPromptModal && selectedSyllabus && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: '600' }}>Generated Prompt</h3>
                <button
                  onClick={() => {
              ...
              <div style={{
                ...
                <p style={{ color: '#374151', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>
                  {selectedSyllabus.generatedPrompt || 'No prompt generated yet. Click "Generate Video" to create a prompt automatically.'}
                </p>
              </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                <strong>Note:</strong> Video generation typically takes 1-2 minutes. You can close this modal and the video will continue generating in the background.
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setVideoOptions({ ...videoOptions, customPrompt: e.target.value })}
                    rows={5}
                    style={{
                      width: '100%',
                      padding: '0.75rem',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setVideoOptions({ ...videoOptions, aspectRatio: e.target.value })}
                      style={{
                        width: '100%',
                        padding: '0.75rem',
                        border: '1px solid #e5e7eb',
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setVideoOptions({ ...videoOptions, mode: e.target.value })}
                      style={{
                        width: '100%',
                        padding: '0.75rem',
                        border: '1px solid #e5e7eb',
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    gap: '0.5rem',
                    padding: '0.75rem',
                    backgroundColor: generatingVideo[selectedSyllabus._id] ? '#9ca3af' : '#8b5cf6',
                    color: 'white',
                    border: 'none',
                    borderRadius: '0.5rem',
                    cursor: generatingVideo[selectedSyllabus._id] ? 'not-allowed' : 'pointer',
                    fontWeight: '500'
                  }}
                >
